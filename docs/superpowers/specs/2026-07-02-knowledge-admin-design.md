...
- 不做用户或 Agent 的知识库绑定 UI；后端目前只有 replace 写接口，没有查询已绑定知识库的 admin 读接口，前端无法可靠回显已选项。
...
- `POST /api/knowledge-bases/users/replace` 与 `POST /api/knowledge-bases/agents/replace` 只补 helper，暂不接 UI。
...
- 绑定 UI 依赖后端新增查询接口后再做，避免只靠 replace 写接口造成误覆盖。
